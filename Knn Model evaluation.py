...
# ✅ Load dataset safely, skipping bad lines
df = pd.read_csv("tmdb_with_moods_enhanced_cleaned.csv", engine='python', on_bad_lines='skip')
...
def recommend_movies_knn(input_year, input_mood, top_n=10):
    ...
    top_movies = filtered_df[filtered_df['mood'] == mood_prediction[0]].sort_values(
        by='cosine_similarity', ascending=False).head(top_n)
    return top_movies[['title', 'genres', 'year', 'overview']].reset_index(drop=True)

# 🌟 Get user input
try:
    year_input = int(input("📅 Enter the year of movie you want to watch: "))
    mood_input = input("🧠 Enter your current mood (e.g., happy, romantic, thrilling): ").strip().lower()
